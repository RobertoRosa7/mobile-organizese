...
const dashboardReducers = createReducer(
  INITIAL_STATES,
  on(actions.SET_LASTDATE_OUTCOME, (states, { payload }) => ({ ...states, lastdate_outcome: payload, })),
  ...
  on(actions.SET_DATES, (states, { payload }) => ({ ...states, graph_dates: payload, })),
  on(actions.SET_GRAPH_OUTCOME_INCOME, (states, { payload }) => ({ ...states, outcome_income: utils.formatterOutcomeIncome(payload), })),
  on(actions.SET_NOTIFICATION_LIST, (states, { payload }) => ({ ...states, notification_list: payload, })),
  on(actions.SET_DASHBOARD, (states, { payload }) => ({...states, registers: utils.returnRegisters(payload.data.results), })),
  on(actions.SET_GRAPH_CATEGORY, (states, { payload }) => {
    const payloadFormated: any = utils.formatDataToGraphCategory({ ...payload, });
    return { ...states, graph_category: payloadFormated, };
  }),
